...
## Recent Changes
- 001-cover-letter-automation: Added Python 3.13 + google-genai (LLM 3-tier), streamlit (UI), psycopg2-binary (DB), dart-fss (DART 사업보고서), firecrawl-py (JD·인재상 크롤링), pdfminer.six (JD PDF 파싱), python-docx (DOCX 포트폴리오 파싱), requests + beautifulsoup4 (보조 스크래핑)
...
## Active Technologies
- Python 3.13 + google-genai (LLM 3-tier), streamlit (UI), psycopg2-binary (DB), dart-fss (DART 사업보고서), firecrawl-py (JD·인재상 크롤링), pdfminer.six (JD PDF 파싱), python-docx (DOCX 포트폴리오 파싱), requests + beautifulsoup4 (보조 스크래핑) (001-cover-letter-automation)
